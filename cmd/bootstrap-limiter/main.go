...
func run() int {
	...
	pflag.StringVarP(&flagCgroup, "cgroup", "c", "", "cgroup to bootstrap")
	...
	if flagCgroup == "" {
		log.Printf("cgroup not specified")
		return failure
	}
	...
	target := filepath.Join(mountpoint, flagCgroup)
	...
	log.Printf("cgroup %v created", flagCgroup)
	...
	log.Printf("access to cgroup %v granted to user '%v'", flagCgroup, runningUser)
	...
	color.Red("please ensure user '%v' is a member of '%v' group in order to be able to set resource limits for Overseer\n", runningUser, group.Name)
...
func haveConsent() bool {
	...
	fmt.Printf("This command will attempt to setup the system to that Overseer can be used to setup system resource limits. Would you like to continue? [yes/no]: ")
